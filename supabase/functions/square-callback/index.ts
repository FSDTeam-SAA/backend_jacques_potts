...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Received Square callback with code and state:", { code, state });
    ...
    const tokenResponse = await fetch("https://connect.squareup.com/oauth2/token", {
      ...
      headers: {
        ...
        "Authorization": `Basic ${btoa(`${squareClientId}:${squareClientSecret}`)}`,
      },
      body: JSON.stringify({
    ...
    return new Response(null, {
      ...
      headers: {
        ...corsHeaders,
        Location: `${Deno.env.get("SUPABASE_URL")}/sell?integration=square&status=success`,
      },
    });
  } catch (error) {
